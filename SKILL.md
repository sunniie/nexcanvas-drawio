...
2. When the user has not explicitly chosen a workflow mode, show the two bundled previews from `assets/examples/architecture-overview.png` and `assets/examples/detailed-request-flow.png`, then ask whether they want **Architecture Overview**, **Detailed Request Flow**, or both. Use the available local-image tool to display them; if image display is unavailable, provide clickable paths and describe each in one sentence.
3. In the same compact question round, ask for the diagram language and intended audience/use. Ask for source material only when it has not already been supplied. Prefer a structured question UI when available; otherwise ask no more than three numbered questions at once.
4. Inspect the chosen source before proposing structure. Return a concise diagram brief containing the real boundaries, proposed zones/tracks, primary events, dependencies, authorities, icon plan, output format, and unresolved assumptions.
5. Pause for confirmation when a new user's choice would materially change the content or composition. If the user explicitly says to decide autonomously or draw immediately, state the inferred brief and proceed without an extra approval round.
6. Derive every boundary, component group, track, and sequence badge from the subject. Never reuse the component count, five-column rhythm, three-track layout, or step count from an example merely because it looks balanced.
...
9. For short connectors, prefer a standalone text vertex instead of Draw.io's automatic edge label. Center it in the gutter, keep it visually associated with the connector, and use the connector color for the text.
