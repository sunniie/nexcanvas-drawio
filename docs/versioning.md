...
Do not replace the body of a pull request generated by Release Please. Its
machine-managed metadata is part of the release handshake and must remain
intact so the merged pull request can be converted into the intended tag and
GitHub Release. Record additional limitations in the tracked release notes or
as a pull-request comment instead.
