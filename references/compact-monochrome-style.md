...
- Number events only when sequence order is necessary. A labeled topology or data-flow map does not need badges.
...
- Place a label beside the longest clear segment, normally 6-12 px from the line and at least 6 px from every shape. Never let the line strike through its own text.
